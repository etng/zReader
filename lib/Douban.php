...
class Douban
{
    ...
    public static function get($api){
        return self::oAuthClient()->get($api);
    }
    public static function direct($api){
        static $d_client;
        if(! $d_client){

            $d_client = new Douban_OAuth();
        }
        return $d_client->get($api);
    }
    public static function getAccessToken($code){
        return self::oAuthClient()->getAccessToken($code);
    }
    public static function getAuthorizeURL(){
        return self::oAuthClient()->getAuthorizeURL(self::$config->api_scope, self::$config->state);
    }
    public static function oAuthClient(){
        static $client;
        if(!$client){
            $client = new Douban_OAuth(array(
...
class Douban_OAuth {
    ...
    private $user_agent = 'Douban PHP SDK';
    public $http_code = 0;
